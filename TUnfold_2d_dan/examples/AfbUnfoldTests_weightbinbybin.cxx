...
void AfbUnfoldTests_weightbinbybin(Int_t iVar = 0, TString TestType = "Pull", Int_t iWeightedBin = 1, Int_t slopeOption = 0, Int_t Nfunction = 0)
{
#ifdef __CINT__
    ...
    TString iWeightedBin_temp = formatFloat(iWeightedBin, "%6.0i");
    iWeightedBin_temp.ReplaceAll(" " , "" );
    //TString file_name = "p1_f" + FunctionName + "_" + acceptanceName;
    TString file_name = "p1_weightedbin" + iWeightedBin_temp + "_" + acceptanceName;
    ...
    for (int k = 0; k < Nlin; k++)
    {
        ...
        for (Int_t i = 0; i < entries; i++)
        {
            ...
            Int_t observableBinNumber = hEmpty->FindBin( observable );
            if (observableBinNumber < 1) observableBinNumber = 1;
            if (observableBinNumber > nbins1D) observableBinNumber = nbins1D;

            Int_t observableMinusBinNumber = -999;
            if ( combineLepMinus )
            {
                observableMinusBinNumber = hEmpty->FindBin( observableMinus );
                if (observableMinusBinNumber < 1) observableMinusBinNumber = 1;
                if (observableMinusBinNumber > nbins1D) observableMinusBinNumber = nbins1D;
            }
            ...
            if ( (acceptanceName == "lepChargeAsym") || (acceptanceName == "lepAzimAsym") || (acceptanceName == "lepAzimAsym2") )
            {
                ...
                if (TestType == "Linearity" && observableBinNumber == iWeightedBin ) weight = weight * (1.0 + slope);
            ...
            else if ( ttmass > 0 )
            {
                ...
                if ( combineLepMinus )
                {
                    ...
                    if (TestType == "Linearity" && observableMinusBinNumber == iWeightedBin ) weight = orig_weight * (1.0 + slope);
    ...
    if ((TestType == "Linearity"))
    {
        ...
        acceptanceName = acceptanceName + "_weightedbin" + iWeightedBin_temp;
