...
void AfbUnfoldExample(TString Var2D = "mtt", double scalettdil = 1., double scalettotr = 1., double scalewjets = 1., double scaleDY = 1., double scaletw = 1., double scaleVV = 1.)
{
    ...
    for (Int_t iVar = 0; iVar < nVars; iVar++)
    {
        ...
        if (Var2D == "mtt")
        {
            ch_data->SetBranchAddress("tt_mass", &obs2D);
        }
        else if (Var2D == "ttrapidity2")
        {
            ch_data->SetBranchAddress("ttRapidity2", &obs2D);
        }
        else if (Var2D == "ttpt")
        {
            ch_data->SetBranchAddress("ttPt", &obs2D);
        }
        ...
        for (Int_t i = 0; i < ch_data->GetEntries(); i++)
        {
            ...
            if ( tmass > 0 )
            {
                fillUnderOverFlow(hData, sign(-asym_centre + observable)*obs2D, weight, Nsolns);
                if (combineLepMinus)
                {
                    fillUnderOverFlow(hData, sign(-asym_centre + observableMinus)*obs2D, weight, Nsolns);
                }
            }
        }
        ...
        for (int iBkg = 0; iBkg < nBkg; ++iBkg)
        {
            ...
            if (Var2D == "mtt")
            {
                ch_bkg[iBkg]->SetBranchAddress("tt_mass", &obs2D);
            }
            else if (Var2D == "ttrapidity2")
            {
                ch_bkg[iBkg]->SetBranchAddress("ttRapidity2", &obs2D);
            }
            else if (Var2D == "ttpt")
            {
                ch_bkg[iBkg]->SetBranchAddress("ttPt", &obs2D);
            }
            ...
            for (Int_t i = 0; i < ch_bkg[iBkg]->GetEntries(); i++)
            {
                ...
                if ( tmass > 0 )
                {
                    fillUnderOverFlow(hBkg, sign(-asym_centre + observable)*obs2D, weight, Nsolns);
                    if (combineLepMinus)
                    {
                        fillUnderOverFlow(hBkg, sign(-asym_centre + observableMinus)*obs2D, weight, Nsolns);
                    }
                }
            }
        }
        ...
        for (Int_t i = 0; i < ch_top->GetEntries(); i++)
        {
            ...
            if ( tmass > 0 )
            {
                fillUnderOverFlow(hMeas, sign(-asym_centre + observable)*obs2D, weight, Nsolns);
                fillUnderOverFlow(hTrue, sign(-asym_centre + observable_gen)*obs2D_gen, weight, Nsolns);
                fillUnderOverFlow(hTrue_vs_Meas, sign(-asym_centre + observable)*obs2D, sign(-asym_centre + observable_gen)*obs2D_gen, weight, Nsolns);
                if ( combineLepMinus )
                {
                    fillUnderOverFlow(hMeas, sign(-asym_centre + observableMinus)*obs2D, weight, Nsolns);
                    fillUnderOverFlow(hTrue, sign(-asym_centre + observableMinus_gen)*obs2D_gen, weight, Nsolns);
                    fillUnderOverFlow(hTrue_vs_Meas, sign(-asym_centre + observableMinus)*obs2D, sign(-asym_centre + observableMinus_gen)*obs2D_gen, weight, Nsolns);
                }
            }
        }
        ...
        gStyle->SetPalette(1);
        hResp->GetXaxis()->SetTitle(yaxislabel + " #times sign(" + xaxislabel + ") (reco)");
        hResp->GetYaxis()->SetTitle(yaxislabel + " #times sign(" + xaxislabel + ") (gen)");
        hResp->Draw("COLZ");
        c_resp->SetLogz();
        c_resp->SaveAs(Var2D + "_Response_2D_" + acceptanceName + Region + ".pdf");
